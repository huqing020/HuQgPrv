...
void	LogImpl(const char *srcFile, int srcLine, const char *strFunction, const LOG_LEVEL *level, const char *fmt, ...){
	va_list		ap;
...
void ActionLogChain( const char *srcFile
					, int srcLine
					...
					, int *pFd )
{
	...
	FormatTime(tmp, &tmNow, TIME_FORMAT_FORMATTED_TIMESTAMP);
	usec = tv.tv_usec / 100;

	isize = snprintf( sLine
					, MAX_LINE_SIZE
					, "%s.%04d %-5s [%5d] (%s#%d) <%s> - %s\n"
					, tmp
					, usec
    ...
    {// 只有在记录日志文件时切换日期文件
        nDate = ( tmNow.tm_year + 1900 ) * 10000 + ( tmNow.tm_mon + 1 ) * 100 + tmNow.tm_mday;
        if( nDate != sLogDate )
        {
            snprintf( tmp, 63, "cp ./log/RunInfo.log ./log/%d_RunInfo.log", sLogDate );
            system( tmp );
            ftruncate( sLogFd, 0 );
            lseek( sLogFd, 0, SEEK_SET );
            sLogDate = nDate;
        }
    }
...
/// <returns>LOG_LEVEL*</returns>
bool	InitialLog( int *pFd, const char *name )
{
	...
	localtime_r( &tv.tv_sec, &tmNow );
	sLogDate = ( tmNow.tm_year + 1900 ) * 10000 + ( tmNow.tm_mon + 1 ) * 100 + tmNow.tm_mday;
	snprintf( sLogFileName, MAX_PATH_LEN, "./log/RunInfo.log" );//, sLogDate );
	...
	{
		if ((*pFd = open(sLogFileName, O_WRONLY | O_CREAT | O_APPEND, F_MODE_RW)) < 0)
		{
