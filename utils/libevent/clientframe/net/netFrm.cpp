...
void    callback_buffEvent_data_read(struct bufferevent *bev, void *ctx){
    netCltFrm *pclt = reinterpret_cast<netCltFrm*>(ctx);
    if (!pclt)
        return;
...
void    netCltFrm::setNetCfg(std::string strAddr, int iPort)
{
    m_strAddr = strAddr;
    m_iPort = iPort;
}

void    netCltFrm::setLogDir(std::string strDir)
{
    if (!strDir.empty())
        m_strLogDir = strDir;
    else{
...
void	netCltFrm::setAliveCfg(int iAliveFlag /*= 0*/, int iIntervalSend /*= 15*/, int iIntervalRcv /*= 60*/)
{
...
bool	netCltFrm::initialModel()
{
    /// init libevent
    m_pEnvBase = event_base_new();
    if (!m_pEnvBase)
    {
        Warn("netCltFrm event_base_new failure, errMsg[%s]", 
...
bool	netCltFrm::exitModel()
{
    return true;
}

bool	netCltFrm::runloop()
{
    if (!checkConn())
    {
#ifdef _MSC_VER
...
bool    netCltFrm::afterConnectSvr()
{
	if (!m_pBufEnvClt)
    {
...
void    netCltFrm::cb_buffEvent_data_read(struct bufferevent *bev, void *ctx)
{
...
void    netCltFrm::cb_buffEvent_data_write(struct bufferevent *bev, void *ctx)
{
    if (IsEmptySendBuf())
        return;
...
void    netCltFrm::cb_buffEvent_event(struct bufferevent *bev, short what, void *ctx)
{
...
bool    netCltFrm::sendMsg(const char* pstrMsg, int iLen){
    if (!pstrMsg || iLen <= 0)
        return false;
...
bool    netCltFrm::sendMsg(const std::string &strMsg){
    return sendMsg(strMsg.c_str(), strMsg.length());
}

bool    netCltFrm::IsEmptySendBuf(){
    FCAutoLock atLock(&m_csLock);
    return m_lstSendBuf.empty();
}

bool    netCltFrm::setBuff(cbuffstt *pstt)
{
    if (!pstt){
        return false;
    }
...
bool    netCltFrm::getBuff(cbuffstt *&pstt)
{
